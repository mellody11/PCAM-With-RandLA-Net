...
class NetworkForCam(nn.Module):
    ...
    def __init__(self, config):
        ...
        if(config.name == 'S3DIS'):
            self.class_weights = DP.get_class_weights('S3DIS')              # 这个权重没用了
            ...
            self.class_weights = DP.get_class_weights('S3DIS')              # 这个权重没用了，但是删掉loss传参会报错
            self.fc0 = nn.Linear(6, 8)
            self.fc0_acti = nn.LeakyReLU()
            self.fc0_bath = nn.BatchNorm1d(8, eps=1e-6, momentum=0.99)
            nn.init.constant_(self.fc0_bath.weight, 1.0)
        ...
        # self.decoder_0 = pt_utils.Conv2d(d_in, d_out, kernel_size=(1,1), bn=True)       # 中间这层mlp也不要了
    
        self.unary_block = pt_utils.Conv2d(d_in, self.config.num_classes)                 # 1*1卷积
    ...
    def forward(self, end_points):
        ...
        # 生成cam
        cam = self.unary_block(f_encoder_list[-1])  # 维度为batch,feature,nums,1的矩阵 （6, 13, 256, 1）
        feature = cam
        
        # GAP
        classification_logit = self.global_average_block(feature)        # 输入是一个 维度为batch,feature,nums,1的矩阵 （6, 13, 256, 1） 输出为一个batch,13的logit

        # 对cam进行上采样
        for j in range(self.config.num_layers):
            f_interp_i = self.nearest_interpolation(feature, end_points['interp_idx'][-j - 1])  
            feature = f_interp_i



        end_points['logits'] = classification_logit     # (batch,13)的logit
        end_points['cam'] = cam                         # torch.Size([6, 13, 256, 1])
        end_points['cam_up'] = feature                  # torch.Size([6, 13, 4096, 1])

        return end_points           # 这个end_points包含了输入和输出的所有数据



    @staticmethod
    def global_average_block(featuremap):
        
        featuremap = torch.squeeze(featuremap, -1)
        featuremap = torch.transpose(featuremap, 1, 2)

        logit = torch.sum(featuremap, dim=1, keepdim=False) / featuremap.shape[1]
...
def get_loss(logits, labels, pre_cal_weights, device):
    ...
    criterion = nn.CrossEntropyLoss(reduction='none')   # 这里不用权重了，因为这个loss算出来也不是为了反向传播，不影响网络的训练
...
def L2Loss(model, alpha):
    ...
    for name, param in model.named_parameters():                    # 获取所有层的参数
        if 'bias' not in name: #一般不对偏置项使用正则
            l2_loss = l2_loss + (0.5 * alpha * torch.sum(torch.pow(param, 2)))
    return l2_loss


def multi_classification_loss(end_points, model):


    l2loss = L2Loss(model, 1e-5) 

    cross_entropy = nn.BCEWithLogitsLoss()
    output_loss = cross_entropy(end_points['logits'], end_points['cloud_label'])        # 感觉loss有点小

    # sigmoidd = nn.Sigmoid()
    # logit = sigmoidd(end_points['logits'])
    # cross_entropy = nn.BCELoss()
    # output_loss2 = cross_entropy(logit, end_points['cloud_label'])                      

    # cross_entropy = nn.MultiLabelSoftMarginLoss()
    # output_loss3 = cross_entropy(end_points['logits'], end_points['cloud_label'])       # 和第一个一样

    # output_loss = output_loss.mean()
    # loss = output_loss + l2loss

    # loss = output_loss + l2loss
    loss = output_loss
    end_points['classiloss'] = output_loss
    end_points['l2loss'] = l2loss

    return loss, end_points


def compute_mutilabel_classification_acc(end_points):           # 这个好像有点问题，必须所有标签都相等才算分类准确

    correct_prediction = torch.round(torch.sigmoid(end_points['logits'])) == torch.round(end_points['cloud_label']) # round是四舍五入的
    # accuracy = torch.mean(correct_prediction.float())

    accuracy = 0
    for i in range(end_points['logits'].shape[0]):
        if torch.sum(correct_prediction[i].int()) == end_points['logits'].shape[1]:
            accuracy += 1

    accuracy = accuracy / end_points['logits'].shape[0]

    end_points['classiacc'] = torch.tensor([accuracy])
    return correct_prediction, end_points
