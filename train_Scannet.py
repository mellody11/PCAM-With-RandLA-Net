...
from RandLANet import NetworkForCam, compute_loss, compute_acc, IoUCalculator, multi_classification_loss, compute_mutilabel_classification_acc, get_loss
from scannetv2_dataset import Scannet, ScannetSampler
import numpy as np
import os, argparse
...
log_file_name = f'log_train_Scannet.txt'
...
training_dataset = ScannetSampler(dataset, 'training')
validation_dataset = ScannetSampler(dataset, 'validation')
...
net = NetworkForCam(cfg)
net.to(device)

# Load the Adam optimizer
optimizer = optim.Adam(net.parameters(), lr=cfg.learning_rate)      # 优化函数可能要换换
...
def train_one_epoch():
    ...
    for batch_idx, batch_data in enumerate(training_dataloader):
        ...
        loss, end_points = multi_classification_loss(end_points, net)
        loss.backward()
        optimizer.step()

        acc, end_points = compute_mutilabel_classification_acc(end_points)
        ...
        if (batch_idx + 1) % batch_interval == 0:
            ...
            #     stat_dict[key] = 0
            log_string('Step %03d Loss %.3f L2Loss %.3f Acc %.2f lr %.5f --- %.2f ms/batch' % (batch_idx + 1, stat_dict['classiloss'] / batch_interval, stat_dict['l2loss'] / batch_interval, stat_dict['classiacc'] / batch_interval, optimizer.param_groups[0]['lr'], 1000 * (t_end - t_start)))
            stat_dict['classiloss'], stat_dict['classiacc'], stat_dict['l2loss'] = 0, 0, 0 
...
def evaluate_one_epoch():
    ...
    for batch_idx, batch_data in enumerate(validation_dataloader):
        ...
        with torch.no_grad():
            ...
            cam_up = torch.transpose(end_points['cam_up'], 1, 2)
            cam_up = torch.squeeze(cam_up, -1)
            cam_up = torch.multiply(cam_up, end_points['cloud_labels_all'])                 # element wise 的相乘
            cam_up = torch.softmax(cam_up, 2)                                               # 这里的softmax是在验证的时候的，并不影响训练
            end_points['cam_up_2'] = cam_up
